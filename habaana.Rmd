...
pdata <- amatch %>% select(GES_TORE, SPIELZEIT) %>% left_join(zaz_h, by = "GES_TORE") %>% 
  left_join(zaz_g, by = "GES_TORE")

pdata %>% plot_ly(x = ~GES_TORE) %>% 
  add_trace(y = ~ZAZ_HEIM / 60, name = "H", 
            type = "bar") %>% 
  add_trace(y = ~ZAZ_GAST * (-1 / 60), name = "G", 
            type = "bar") %>% 
